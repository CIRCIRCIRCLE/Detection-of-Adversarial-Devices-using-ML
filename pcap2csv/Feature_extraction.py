...
class Feature_extraction():
    ...
    def pcap_evaluation(self,pcap_file,csv_file_name):
        ...
        # Process numeric_df and non_numeric_df separately if needed
        # Create summaries for numeric columns
        df_summary_list = []
        n_rows = 10
        for i in range(0, len(numeric_df), n_rows):
            sliced_df = numeric_df.iloc[i:i + n_rows]
            df_summary_list.append(sliced_df.mean().to_frame().T)

        numeric_summary_df = pd.concat(df_summary_list).reset_index(drop=True)

        # Combine numeric summaries with non-numeric data (if any)
        if not non_numeric_df.empty:
            final_df = pd.concat([numeric_summary_df, non_numeric_df.reset_index(drop=True)], axis=1)
        else:
            final_df = numeric_summary_df
