...
def build_LSTM_CNN_model(input_shape):
    model = Sequential([
        LSTM(50, return_sequences=True, input_shape=input_shape),  # LSTM layer with return_sequences=True
        Dropout(0.5),
        Conv1D(filters=64, kernel_size=3, activation='relu'),       # Conv1D after LSTM
        MaxPooling1D(pool_size=2),
        Flatten(),                                                  # Flatten needed before the Dense layer
...
model = build_LSTM_CNN_model((X_train.shape[1], 1))
