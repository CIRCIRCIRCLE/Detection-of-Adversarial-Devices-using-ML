...
def cross_validate_model(data, label_encoder, config):
    ...
    for train, test in kfold.split(X, y):
        ...
        if config['classifier_type'] == "CNN":
        ...
        elif config['classifier_type'] == "CNN_LSTM":
            X_train_encoded = X_train_encoded.reshape((X_train_encoded.shape[0], 1, encoding_dim, 1))
            X_test_encoded = X_test_encoded.reshape((X_test_encoded.shape[0], 1, encoding_dim, 1))
            classifier = CNN_LSTM_classifier(X_train_encoded, y[train], encoding_dim, label_encoder, config['ClassifierEpochs'], config['batch_size'])
        elif config['classifier_type'] == "LSTM_CNN":
            ...
            classifier = LSTM_CNN_classifier(X_train_encoded, y[train], X_train_encoded.shape[1], label_encoder, config['ClassifierEpochs'], config['batch_size'])
        else:
    ...
    avg_scores = {metric: np.mean([score[metric] for score in scores]) for metric in scores[0]}
    std_scores = {metric: np.std([score[metric] for score in scores]) for metric in scores[0]}

    #logging.info("Average Scores: %s", avg_scores)
    #logging.info("Standard Deviation of Scores: %s", std_scores)

    return avg_scores, std_scores, reports
...
def main():
    ...
    avg_scores, std_scores, reports = cross_validate_model(data, label_encoder, config)
